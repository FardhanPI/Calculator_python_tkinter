...
def resize_element(event=None):
    ...
    element.place(x=root.winfo_width()*0.025, y=root.winfo_height()*0.02)
    # Configure the size of the element
    element.configure(height=element_height, width=element_width)
    ...
    element.create_text(root.winfo_width()*0.95,20, anchor="e", text="", fill="black", font=("Digital-7",35))

    return element_height, element_width


def sevenbutton(element):

    element_width, element_height = resize_element()  # Obtain initial values

    print("oogjogkjgkj")
    element.create_text(root.winfo_width()*0.95,root.winfo_height()*0.1, anchor="e", text="7", fill="black", font=("Digital-7",35))
